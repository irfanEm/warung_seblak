<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center space-y-4 sm:space-y-0">
        ...
        <a href="{{ route('admin.table.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition shadow-sm font-medium">
            + Tambah Meja
        </a>
    </div>
    ...
    @if (session()->has('message'))
        <div class="p-4 rounded-md {{ session('message_type') == 'success' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800' }}">
            {{ session('message') }}
        </div>
    @endif
    ...
    <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nomor meja..." class="w-full sm:w-1/3 rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
    </div>
