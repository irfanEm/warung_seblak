<div class="px-4 py-8">
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 text-center">
        ...
        <div class="bg-gray-50 rounded-xl p-4 mb-6 text-left">
            <p class="text-xs text-gray-500 font-medium mb-1">Kode Pesanan</p>
            <p class="text-sm font-bold text-gray-900 mb-3">{{ $order->tracking_code }}</p>
        ...
        document.getElementById('pay-button').onclick = function () {
            window.snap.pay('{{ $snapToken }}', {
                onSuccess: function(result) {
                    window.location.href = '{{ route('customer.checkout.finish', ['tracking_code' => $order->tracking_code]) }}';
                },
                onPending: function(result) {
                    window.location.href = '{{ route('customer.checkout.finish', ['tracking_code' => $order->tracking_code]) }}';
                },
                onError: function(result) {
                    window.location.href = '{{ route('customer.checkout.error', ['tracking_code' => $order->tracking_code]) }}';
                },
                onClose: function() {
                    window.location.href = '{{ route('customer.checkout.unfinish', ['tracking_code' => $order->tracking_code]) }}';
                }
            });
        };
    </script>
